...
function Marcas(props) {
    ///URLs de la api
    const getUrl="https://localhost:44362/api/marca";
    ...
    const eliminar=async()=>{
        gestorSeleccionado.id_Marca = parseInt(gestorSeleccionado.id_Marca);
        axios.delete(getUrl+"/"+gestorSeleccionado.id_Marca).then(response=>{
            setData(data.filter(gestorSeleccionado => gestorSeleccionado.id_Marca !== response.data))
            handleCloseBorrar();
            limpiar();
            consultarDatos();
        }).catch(error=>{
                    ...
                    <tbody>
                        {data.map(marcas =>{
                                return(
                                    <>
                                        <tr>
                                            <th> {marcas.id_Marca} </th>
                                            <th> {marcas.mar_Nombre} </th>
                                            <th> {marcas.fechaCreacion} </th>
                                            <th> {marcas.fechaModificacion} </th>
                                            <th> <Button variant="outline-success" onClick={()=>seleccionarDato(marcas)}> Editar </Button></th>
                                            <th> <button className="btn btn-danger" onClick={()=>seleccionarDatoElim(marcas)}> Eliminar </button></th>
                                        </tr>
                                    </>
                                )
                            })}
